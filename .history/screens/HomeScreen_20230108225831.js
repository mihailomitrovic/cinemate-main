...
const HomeScreen = () => {
  ...
  const getMovies1 = async () =>{
    const querySnapshot = await getDocs(query(collection(db, "movies"), orderBy('name', 'desc')));
    setMovies(querySnapshot.docs);
  }

  const getMovies2 = async () =>{
    const querySnapshot = await getDocs(query(collection(db, "movies"), orderBy('runtime', 'desc')));
    setMovies(querySnapshot.docs);
  }
