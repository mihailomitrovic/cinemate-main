...
const HomeScreen = () => {
  ...
  return (
    <View style = {styles.background}>
      ...
      <Text style = {{fontWeight: '500', fontSize: 13}}>Order by: </Text>
        <TouchableOpacity style = {filter === 'name' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('name'); getMovies(filter)}}>
        <Text style = {filter === 'name' ? styles.pickTextA : styles.pickTextB}>Title</Text>
        </TouchableOpacity>
        <TouchableOpacity style = {filter === 'runtime' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('runtime'); getMovies(filter)}}>
        <Text style = {filter === 'runtime' ? styles.pickTextA : styles.pickTextB}>Runtime</Text>
        </TouchableOpacity>
        <TouchableOpacity style = {filter === 'year' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('year'); getMovies(filter)}}>
