...
const HomeScreen = () => {
  ...
  const getMovies1 = async (filter) =>{
    const querySnapshot = await getDocs(query(collection(db, "movies"), orderBy(filter.data().name)));
  ...
  return (
    <View style = {styles.background}>
      ...
      <View style = {styles.pickContainerA}>
        <TouchableOpacity style = {filter === 'name' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('name'); getMovies1('name')}}>
        <Text style = {filter === 'name' ? styles.pickTextA : styles.pickTextB}>Title</Text>
        </TouchableOpacity>
        <TouchableOpacity style = {filter === 'runtime' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('runtime'); getMovies1('runtime')}}>
        <Text style = {filter === 'runtime' ? styles.pickTextA : styles.pickTextB}>Runtime</Text>
        </TouchableOpacity>
        <TouchableOpacity style = {filter === 'year' ? styles.pickA : styles.pickB} onPress = {() => {setFilter('year'); getMovies1('year')}}>
