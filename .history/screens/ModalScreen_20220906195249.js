...
import { doc, setDoc, updateDoc, arrayUnion, where, onSnapshot} from '@firebase/firestore'
...
const ModalScreen = ({route}) => {
  ...
  const bookMovie = async() => {
    ...
    const bookingname = db.collection('booking').doc('ElvisMonday17:00')

    const doc = await bookingname.get();
    if (!doc.exists) {
  ...
  try {
    await updateDoc(doc(db, 'booking', bookingid), {
     ...
     navigation.navigate('Match', {id: bookingid});
    }
    )
  }
  catch(e) {
  ...
  return (
    <View style = {styles.container}>
      ...
      <View style = {styles.info}>
        <Image source = {route.params.pic} style = {styles.image}/>
      ...
      <View style={[styles.buttonContainer, {opacity: incompleteForm ? 0.5 : 1.0}]}>
        <TouchableOpacity onPress = {bookMovie} style = {styles.button}>
          <Text style = {styles.buttonText}>Find a mate</Text>
        </TouchableOpacity>
      </View>
    </View>
  )
