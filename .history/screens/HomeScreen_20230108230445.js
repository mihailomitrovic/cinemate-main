...
const HomeScreen = () => {
  ...
  const getMovies = async (title) =>{
    const querySnapshot = await getDocs(query(collection(db, "movies"), orderBy(title)));
  ...
  return (
    <View style = {styles.background}>
      ...
      <View style = {styles.pickContainerA}>
        <TouchableOpacity style = {styles.pickA} onPress = {() => {setTitle('title'); getMovies(title);}}>
        <Text style = {styles.pickTextA}>Name</Text>
        </TouchableOpacity>
        <TouchableOpacity style = {styles.pickA} onPress = {() => {setTitle('runtime'); getMovies(title);}}>
