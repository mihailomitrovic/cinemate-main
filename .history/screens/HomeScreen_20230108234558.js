...
const HomeScreen = () => {
  ...
  const getMovies = async (filter) =>{
    const querySnapshot = await getDocs(query(collection(db, "movies"), orderBy(filter.filterName)));
    setMovies(querySnapshot.docs);
  }
  ...
  return (
    <View style = {styles.background}>
      ...
      <View style = {styles.pickContainerA}>
        <TouchableOpacity style = {styles.pickA} onPress = {() => {setFilter(filter.namgetMovies1('name')}}>
